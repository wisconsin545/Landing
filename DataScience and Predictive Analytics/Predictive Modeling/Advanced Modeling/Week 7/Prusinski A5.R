# Prusinski A5 - Mathematical Programming  
# This code is adjusted for Tuesday instead of Wednesday 
...
load("mtpa_split_plotting_utilities.Rdata")# run Appendix C3 first of Dr. Miller's Book 
load("mtpa_wait_time_ribbon_utility.Rdata")# run Appendix C4 firs of Dr. Miller's Book
...
# read in call center data 
cc.feb <- read.csv("data_anonymous_bank_february.txt")
# examine the structure of the call center data
print(str(cc.feb))
...
tuesdays <- subset(call.center.data, subset = (day_of_week == "Tuesday"))
...
# 24-hour clock has first hour coded as zero in input data file
  ...
  this.hour.calls <- 
    subset(tuesdays, subset = (call_hour == coded.index.for.hour))  
...
hourly.arrival.rate <- calls.for.hour/4  # four Tuesdays in February
...
tuesdays.served <- subset(tuesdays, subset = (server != "NO_SERVER"))
...
# 24-hour clock has first hour coded as zero in input data file
  coded.index.for.hour <- index.for.hour - 1  
  this.hour.calls <- 
    subset(tuesdays.served, subset = (call_hour == coded.index.for.hour))
  if(nrow(this.hour.calls) > 0) {
...
hourly.served.rate <- served.for.hour/4  # four Tuesdays in February 1999
...
pdf(file = "fig_operations_management_tuesdays_arrived_served.pdf", 
  width = 11, height = 8.5)
...
# and noting the effect upon the optimal schedule of workers to shifts
