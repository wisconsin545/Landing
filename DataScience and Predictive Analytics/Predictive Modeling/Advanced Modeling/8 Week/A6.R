#Daniel Prusinski Programming Assignment 6: Spatial Regression Modeling 
...
Alco <- density(houses$value)
Ma <- density(houses$income)
As <- density(houses$age)
AlcAs <- density(houses$rooms/houses$pop)

Mg <- density(houses$bedrooms / houses$pop)
Ph <- density(houses$pop / houses$hh)
Fl <- density(houses$hh)
...
AlcoL <- density(houses$log_value)
MaL <- density(houses$income_squared)
AsL <- density(houses$log_age)
AlcAsL <- density(houses$log_pc_rooms)

#Lets try the layout function
layout(matrix(c(1,2,3,4), 2, 2, byrow = TRUE))
plot (AlcoL)
polygon(AlcoL, col="red", border="blue")
plot (MaL)
polygon(MaL, col="red", border="blue")
plot (AsL)
polygon(AsL, col="red", border="blue")
plot (AlcAsL)
polygon(AlcAsL, col="red", border="blue")

MgL <- density(houses$log_pc_bedrooms)
PhL <- density(houses$log_pop_hh)
FlL <- density(houses$log_hh)

layout(matrix(c(1,2,3,4), 2, 2, byrow = TRUE))
plot (MgL)
polygon(MgL, col="red", border="blue")
plot (PhL)
polygon(PhL, col="red", border="blue")
plot (FlL)
polygon(FlL, col="red", border="blue")
...
# here we are selecting the San Francisco region
BB.TOP <- 37.88
BB.BOTTOM <- 36.96
BB.RIGHT <- -121.90
BB.LEFT <- -122.38
...
# try alternative spatial data models... 
# spatial grid or lattice structures are possible

# explore the data further using data visualization... maps

# run a full benchmark study by evaluating models within a loop
# run a few hundred iterations within the benchmark study

# work on another metropolitan area in California by specifying 
# different area selection values for longitude and latitude
