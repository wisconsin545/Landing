...
#Box Plots - Define the variables
Alcohol <-(wine$Alcohol)
Malic <- (wine$MalicAcid)
Ash <- (wine$Ash)
AlcAsh <- (wine$AlcAsh)
Mg <- (wine$Mg)
Phenols <- (wine$Phenols)
Flav <- (wine$Flav)
NonFlaPhe <- (wine$NonFlavPhenols)
Proa <- (wine$Proa)
Color <- (wine$Color)
Hue <- (wine$Hue)
OD <- (wine$OD)
...
#These two will not work based on the ternary values
ClassDigit <- (wine$classdigit)
Class <- (wine$class)

#Make the box plots
boxplot (Malic, Ash, Phenols, Flav, Proa, OD)
# 1= Malic, 2= Ash, 3= Phenols, 4= Flav, 5= Proa, 6= OD,
boxplot (Alcohol, AlcAsh, Color)
# 1= Alcohol, 2=AlAsh, 3= Color
boxplot (Proline)
1= Proline
boxplot (NonFlaPhe , Hue)
# 1=NonFlaPhe, 2=Hue 
boxplot (Mg)
# 1= Mg
