...
* Need to standardize the data before performing any type of "component" analysis;
...
* What is the correlation structure between the principal components?;



/*
* Two examples that do not work.  Note why these do not work.;
ods graphics on;
...
* Here is a factor analysis that "worked".  Still, what is wrong with this output?;
ods graphics on;
proc factor data=xdata method=uls heywood out=xdata_uls outstat=uls_stats
...
ods graphics on;
proc factor data=xdata method=uls heywood rotate=varimax out=xdata_varimax outstat=varimax_stats
	mineigen=0 priors=max nfactors=5 score scree ;
run;
ods graphics off;


* What is the correlation structure between the factor components?;
...
/* 	Fit regression models with the first 5 principal components and the first 5 factor 
	components and compare and contrast the models.  Is one model clearly better than
	the other?
*/
